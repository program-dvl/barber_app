...
const props = defineProps({ businessLabel: String, subscription: Object, trial: Object, plans: Array, planRanks: Object, entitlements: Object, invoices: Array, payments: Array, exportAvailable: Boolean, checkoutStatus: String, checkoutAttempt: Object, pendingChange: Object, billingReadiness: Object });
const page = usePage();
...
const planChangeKind = plan => {
    ...
    if (props.subscription.billing_interval === 'annual' && plan.price.billing_interval === 'monthly') return 'scheduled_interval_change';
    if (planRank(plan.code) < planRank(props.subscription.plan.code)) return 'downgrade';
    return 'upgrade';
};
const planActionLabel = plan => ({
    current: 'Your current plan',
    unavailable: props.pendingChange ? 'Change pending' : 'Resolve billing status first',
    scheduled_interval_change: 'Switch at renewal',
...
const selectedChangeDescription = computed(() => {
    ...
    if (['downgrade', 'scheduled_interval_change'].includes(selectedChangeKind.value)) return `Your ${props.subscription.plan.name} access continues through ${date(props.subscription.current_period_ends_at)}. ${selectedPlan.value.name} starts at renewal; no records are deleted.`;
    return `Stripe will apply ${selectedPlan.value.name} with a prorated invoice. Signed Stripe confirmation updates your access.`;
});
...
const changePlan = async () => {
    ...
    try {
        const response = await axios.post(route('business.billing.plan-change', page.props.tenant.public_id), {
            ...
            timing: ['downgrade', 'scheduled_interval_change'].includes(selectedChangeKind.value) ? 'period_end' : 'immediate',
            reason: `Owner self-service ${selectedChangeKind.value}.`,
        });
        actionNotice.value = response.data.message;
        planDialog.value?.close();
        router.reload({
            only: ['subscription', 'pendingChange', 'entitlements', 'invoices', 'payments'],
...
<template>
    <AppLayout title="Subscription & billing" :business-label="businessLabel">
        ...
        <section class="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,0.75fr)]">
            <article class="relative overflow-hidden rounded-2xl bg-[var(--brand-primary)] px-6 py-7 text-white shadow-[var(--shadow-raised)] sm:px-8">
        ...
        <AppDialog id="plan-change" ref="planDialog" :title="selectedChangeKind === 'downgrade' ? `Schedule ${selectedPlan?.name || 'plan'}?` : `Change to ${selectedPlan?.name || 'plan'}?`" :description="selectedChangeDescription" :confirm-label="busy ? 'Updating with Stripe…' : (selectedChangeKind === 'downgrade' ? 'Schedule downgrade' : 'Confirm upgrade')" :confirm-disabled="busy" :close-on-confirm="false" @confirm="changePlan">
        ...
        <AppDialog id="cancel-subscription" ref="cancelDialog" title="Cancel at the end of this billing period?" :description="`Your ${subscription.plan.name} access continues until ${date(subscription.current_period_ends_at)}. Future renewals stop, and your business records are retained.`" :confirm-label="busy ? 'Scheduling cancellation…' : 'Cancel at renewal'" :confirm-disabled="busy" :close-on-confirm="false" destructive @confirm="confirmCancellation">
            <p v-if="error && errorArea === 'subscription'" class="mb-4 rounded-lg bg-[var(--status-danger-soft)] p-3 text-sm text-[var(--status-danger)]" role="alert">{{ error }}</p>
            <label class="block text-sm font-semibold text-[var(--text-strong)]">Main reason <select v-model="cancellationReason" class="cd-input mt-2 block w-full"><option>No longer needed</option><option>Too expensive</option><option>Missing a feature</option><option>Closing the business</option><option>Switching to another product</option><option>Other</option></select></label>
        </AppDialog>
    </AppLayout>
</template>
