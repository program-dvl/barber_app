...
const faq = computed(() => [
    ...
    { question: 'Are appointment payments included?', answer: 'No. Paddle is used for the ClipperDesk SaaS subscription. Client deposits and appointment payments are a separate salon commerce flow and live card collection remains provider-qualified.' },
    ...
    <HomeLayout>
        ...
        <section class="cd-public-section border-b border-[var(--border-subtle)]">
            <PublicContainer>
                ...
                <p class="cd-eyebrow mt-10">Pricing</p>
                ...
                <p class="mt-7 max-w-3xl text-lg leading-8 text-[var(--text-muted)] sm:text-xl">ClipperDesk uses a {{ catalog.trial_days }}-day verified trial. Paid prices and allowances below appear only when the complete, effective Paddle catalog is available.</p>
            </PublicContainer>
        </section>

        <section class="cd-public-section">
            <PublicContainer>
                <div v-if="!catalog.available" class="mx-auto max-w-3xl rounded-[var(--radius-xl)] border border-[var(--status-warning)]/30 bg-[var(--surface-raised)] p-7 sm:p-10" role="status">
                ...
                <template v-else>
                    ...
                    <div class="mt-12 grid gap-6 lg:grid-cols-2" aria-live="polite">
                        <article v-for="plan in catalog.plans" :key="plan.code" class="cd-marketing-card flex flex-col">
        ...
        <section class="cd-public-section bg-[var(--surface-subtle)]">
            <PublicContainer>
                <SectionHeading eyebrow="Commercial clarity" title="Subscription billing is separate from salon payments" description="Paddle owns the ClipperDesk SaaS checkout frame. Appointment deposits, retail tenders and client payment evidence belong to each salon's operating workflow and are not bundled into the subscription price." />
                <div class="mt-9 max-w-4xl rounded-[var(--radius-lg)] border-l-4 border-[var(--status-warning)] bg-white p-6 leading-7 text-[var(--text-muted)]"><strong class="text-[var(--text-strong)]">Launch qualification:</strong> live Paddle checkout remains blocked until the production domain, seller identity, credentials, price mappings, webhook and settlement path are certified. This page never opens a sandbox checkout. Review the proposed <Link :href="route('refund.show')" class="font-bold text-[var(--action-primary)] underline underline-offset-4">refund and cancellation policy</Link> before paid checkout.</div>
            </PublicContainer>
        </section>
