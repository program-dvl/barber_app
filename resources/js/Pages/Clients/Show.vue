...
const visibleTabs = computed(() => tabs.filter(tab => tab.id !== 'privacy' || props.permissions.privacy));
...
<template>
    <AppLayout :title="client.name" :business-label="businessLabel">
        ...
        <div ref="tabList" class="cd-section-nav mt-5" role="tablist" aria-label="Client profile sections" @keydown="moveTab"><button v-for="tab in visibleTabs" :id="`client-tab-${tab.id}`" :key="tab.id" type="button" class="cd-section-nav-item" role="tab" :tabindex="activeTab === tab.id ? 0 : -1" :aria-selected="activeTab === tab.id" aria-controls="client-section-panel" @click="activeTab = tab.id">{{ tab.label }}</button></div>
        ...
        <div v-show="activeTab === 'overview'" class="mt-6 grid gap-6 xl:grid-cols-2">
            <SurfaceCard title="Profile and preferences" description="Contact details, service preferences, and helpful context for future visits.">
                <form v-if="permissions.update" class="grid gap-4 sm:grid-cols-2" @submit.prevent="saveProfile">
                    ...
                    <label class="text-sm font-medium">Preferred employee<AppSelect v-model="profile.preferred_staff" class="cd-input mt-1 w-full"><option :value="null">No preference</option><option v-for="staff in staffOptions" :key="staff.public_id" :value="staff.public_id">{{ staff.display_name }}</option></AppSelect></label>
                    <label class="text-sm font-medium">Preferred services<AppSelect v-model="profile.preferred_services" class="cd-input mt-1 min-h-24 w-full" multiple><option v-for="service in serviceOptions" :key="service.public_id" :value="service.public_id">{{ service.name }}</option></AppSelect></label>
                    <fieldset class="sm:col-span-2"><legend class="text-sm font-medium">Tags</legend><div v-if="tags.length" class="mt-2 flex flex-wrap gap-2"><button v-for="tag in tags" :key="tag" type="button" class="cd-status gap-1 bg-[var(--surface-subtle)] text-[var(--text-strong)]" :aria-label="`Remove ${tag} tag`" @click="tags = tags.filter(item => item !== tag)">{{ tag }} <XMarkIcon class="size-3.5" aria-hidden="true" /></button></div><div class="mt-2 flex gap-2"><input v-model="tagDraft" class="cd-input" placeholder="Add a tag" @keydown.enter.prevent="addTag"><AppButton type="button" variant="secondary" @click="addTag">Add</AppButton></div></fieldset>
                    ...
                    <label class="text-sm font-medium sm:col-span-2">How they found you<AppSelect v-model="profile.referral_source" class="cd-input mt-1"><option :value="null">Not recorded</option><option v-if="profile.referral_source && !['walk_in','friend_or_family','google','instagram','facebook','other'].includes(profile.referral_source)" :value="profile.referral_source">{{ profile.referral_source }}</option><option value="walk_in">Walk-in</option><option value="friend_or_family">Friend or family</option><option value="google">Google</option><option value="instagram">Instagram</option><option value="facebook">Facebook</option><option value="other">Other</option></AppSelect></label>
                    <label class="text-sm font-medium sm:col-span-2">Why are you updating this profile?<AppSelect v-model="profile.reason" class="cd-input mt-1" required><option value="" disabled>Choose a reason</option><option value="Client asked us to correct their details.">Client requested a correction</option><option value="Updated during an appointment or visit.">Updated during a visit</option><option value="Administrative data quality correction.">Administrative correction</option></AppSelect></label>
            ...
            <SurfaceCard title="Safety notes" description="Allergies, formulas, patch tests, treatment context, preferences, and important warnings retain their author.">
                ...
                <form v-if="permissions.addNote" class="mt-4 grid gap-3" @submit.prevent="addNote">
                    <div class="grid gap-3 sm:grid-cols-2"><label class="text-sm font-medium">Note type<AppSelect v-model="note.kind" class="cd-input mt-1 w-full"><option v-for="kind in ['general','allergy','sensitivity','formula','hair','skin','treatment','patch_test','preference','warning']" :key="kind" :value="kind">{{ humanLabel(kind) }}</option></AppSelect></label><label class="text-sm font-medium">Visibility<AppSelect v-model="note.visibility" class="cd-input mt-1 w-full"><option value="standard">Standard</option><option v-if="permissions.sensitive" value="sensitive">Sensitive</option></AppSelect></label></div>
                    <label class="text-sm font-medium">Note content<textarea v-model="note.content" class="cd-input mt-1 min-h-28 w-full" placeholder="Add client context" required /></label>
        ...
        <SurfaceCard v-if="permissions.forms" v-show="activeTab === 'forms'" class="mt-6" title="Form template builder" description="When wording changes, ClipperDesk publishes a new version while completed forms keep exactly what the client saw.">
            <form class="grid gap-4" @submit.prevent="publishTemplate">
                <div class="grid gap-3 sm:grid-cols-2">
                    <label class="text-sm font-medium">Start or revise<AppSelect v-model="formBuilder.template" class="cd-input mt-1 w-full" @change="loadTemplate"><option value="">New template</option><option v-for="template in formTemplates" :key="template.public_id" :value="template.public_id">{{ template.name }} · v{{ template.current_version }}</option></AppSelect></label>
                    <label class="text-sm font-medium">Internal name<input v-model="formBuilder.name" class="cd-input mt-1 w-full" required></label>
                    <label class="text-sm font-medium">Purpose<AppSelect v-model="formBuilder.purpose" class="cd-input mt-1" required><option value="consultation">Consultation</option><option value="treatment">Treatment</option><option value="allergy">Allergy</option><option value="consent">Consent</option><option value="intake">Client intake</option><option value="other">Other</option></AppSelect></label>
                    <label class="text-sm font-medium">Client-facing title<input v-model="formBuilder.title" class="cd-input mt-1 w-full" required></label>
                    <label class="text-sm font-medium sm:col-span-2">Introduction<textarea v-model="formBuilder.introduction" class="cd-input mt-1 min-h-24 w-full" /></label>
                    <label class="text-sm font-medium sm:col-span-2">Associated services<AppSelect v-model="formBuilder.services" class="cd-input mt-1 min-h-24 w-full" multiple><option v-for="service in serviceOptions" :key="service.public_id" :value="service.public_id">{{ service.name }}</option></AppSelect></label>
                </div>
        ...
        <div v-show="['forms', 'files'].includes(activeTab)" class="mt-6 grid gap-6 xl:grid-cols-2">
            <SurfaceCard v-show="activeTab === 'forms'" title="Forms and consent" description="Review requested and completed forms, or send a secure form link for this client.">
                ...
                <form v-if="permissions.forms && formTemplates.length" class="grid gap-3" @submit.prevent="requestForm"><label class="text-sm font-medium">Form template<AppSelect v-model="formRequest.template" class="cd-input mt-1 w-full"><option v-for="template in formTemplates" :key="template.public_id" :value="template.public_id">{{ template.name }} · v{{ template.current_version }}</option></AppSelect></label><label class="text-sm font-medium">Originating appointment<AppSelect v-model="formRequest.appointment" class="cd-input mt-1 w-full"><option value="">No appointment link</option><option v-for="visit in appointments" :key="visit.public_id" :value="visit.public_id">{{ visit.reference }}</option></AppSelect></label><AppButton type="submit">Create secure form request</AppButton></form>
            ...
            <SurfaceCard v-show="activeTab === 'files'" title="Private files" description="Store visit photos and documents privately. Download links expire automatically.">
                ...
                <form v-if="permissions.attachments" class="grid gap-3" @submit.prevent="upload"><label class="text-sm font-medium">Choose attachment<input class="mt-1 block min-h-11 w-full" type="file" accept="image/jpeg,image/png,application/pdf" required @change="attachment.attachment = $event.target.files[0]"></label><div class="grid gap-3 sm:grid-cols-2"><label class="text-sm font-medium">Attachment type<AppSelect v-model="attachment.kind" class="cd-input mt-1 w-full"><option value="file">File</option><option value="before">Before photo</option><option value="after">After photo</option><option value="profile_photo">Profile photo</option></AppSelect></label><label class="text-sm font-medium">Attachment visibility<AppSelect v-model="attachment.visibility" class="cd-input mt-1 w-full"><option value="standard">Standard</option><option v-if="permissions.sensitive" value="sensitive">Sensitive</option></AppSelect></label></div><AppButton type="submit">Store private attachment</AppButton></form>
            </SurfaceCard>
        </div>
        ...
        <div v-if="permissions.merge && duplicates.length" v-show="activeTab === 'overview'" class="mt-6">
            <SurfaceCard title="Possible duplicates" description="Compare the evidence before choosing which profile to keep. ClipperDesk never merges a possible match automatically.">
                ...
                <div v-if="mergePreview" class="mt-4 rounded-xl border border-[var(--status-warning)] bg-[var(--status-warning-soft)] p-4"><h3 class="font-semibold">Merge preview</h3><ul class="mt-2 text-sm"><li v-for="(count, relation) in mergePreview.evidence.relationship_counts" :key="relation">{{ relation.replaceAll('_', ' ') }}: {{ count }}</li></ul><label class="mt-3 block text-sm font-medium">Required merge reason<input v-model="mergeReason" class="cd-input mt-1 w-full"></label><AppButton class="mt-3" :disabled="!mergeReason.trim()" @click="confirmMerge">Confirm survivor and merge</AppButton></div>
            </SurfaceCard>
        </div>
        ...
        <SurfaceCard v-if="permissions.privacy" v-show="activeTab === 'privacy'" class="mt-6" title="Privacy requests" description="Record and track exports, corrections, consent withdrawals, and deletion or anonymisation reviews.">
            <form class="grid gap-3 sm:grid-cols-2" @submit.prevent="submitPrivacy">
                <label class="text-sm font-medium">Request type<AppSelect v-model="privacy.type" class="cd-input mt-1 w-full"><option value="export">Data export</option><option value="correction">Correction</option><option value="consent_withdrawal">Consent withdrawal</option><option value="deletion_anonymization">Deletion / anonymisation review</option></AppSelect></label>
