...
<template>
    <AppLayout :title="client.name" :business-label="businessLabel">
        ...
        <nav class="cd-section-nav mt-5" aria-label="Client profile sections"><button v-for="tab in tabs" :key="tab.id" v-show="tab.id !== 'privacy' || permissions.privacy" type="button" class="cd-section-nav-item" role="tab" :aria-selected="activeTab === tab.id" @click="activeTab = tab.id">{{ tab.label }}</button></nav>
        ...
        <div v-show="activeTab === 'overview'" class="mt-6 grid gap-6 xl:grid-cols-2">
            <SurfaceCard title="Profile and preferences" description="Contact details, service preferences, and helpful context for future visits.">
                <form v-if="permissions.update" class="grid gap-4 sm:grid-cols-2" @submit.prevent="saveProfile">
                    ...
                    <label class="text-sm font-medium">Preferred services<select v-model="profile.preferred_services" class="cd-input mt-1 min-h-24 w-full" multiple><option v-for="service in serviceOptions" :key="service.public_id" :value="service.public_id">{{ service.name }}</option></select></label>
            ...
            <SurfaceCard title="Safety notes" description="Allergies, formulas, patch tests, treatment context, preferences, and important warnings retain their author.">
                ...
                <form v-if="permissions.addNote" class="mt-4 grid gap-3" @submit.prevent="addNote">
                    <div class="grid gap-3 sm:grid-cols-2"><label class="text-sm font-medium">Note type<select v-model="note.kind" class="cd-input mt-1 w-full"><option v-for="kind in ['general','allergy','sensitivity','formula','hair','skin','treatment','patch_test','preference','warning']" :key="kind" :value="kind">{{ humanLabel(kind) }}</option></select></label><label class="text-sm font-medium">Visibility<select v-model="note.visibility" class="cd-input mt-1 w-full"><option value="standard">Standard</option><option v-if="permissions.sensitive" value="sensitive">Sensitive</option></select></label></div>
        ...
        <SurfaceCard v-if="permissions.forms" v-show="activeTab === 'forms'" class="mt-6" title="Form template builder" description="When wording changes, ClipperDesk publishes a new version while completed forms keep exactly what the client saw.">
            <form class="grid gap-4" @submit.prevent="publishTemplate">
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    <label class="text-sm font-medium sm:col-span-2">Associated services<select v-model="formBuilder.services" class="cd-input mt-1 min-h-24 w-full" multiple><option v-for="service in serviceOptions" :key="service.public_id" :value="service.public_id">{{ service.name }}</option></select></label>
                </div>
                <fieldset><legend class="font-semibold">Fields</legend><div class="mt-3 space-y-3">
                    <div v-for="(field, index) in formBuilder.fields" :key="index" class="grid gap-3 rounded-xl border border-[var(--border-subtle)] p-3 sm:grid-cols-2">
                        ...
                        <label class="text-sm font-medium">Answer type<select v-model="field.type" class="cd-input mt-1 w-full"><option v-for="type in ['text','number','date','yes_no','multiple_choice','signature']" :key="type" :value="type">{{ humanLabel(type) }}</option></select></label>
        ...
        <SurfaceCard v-if="permissions.privacy" v-show="activeTab === 'privacy'" class="mt-6" title="Privacy requests" description="Record and track exports, corrections, consent withdrawals, and deletion or anonymisation reviews.">
            <form class="grid gap-3 sm:grid-cols-2" @submit.prevent="submitPrivacy">
                <label class="text-sm font-medium">Request type<select v-model="privacy.type" class="cd-input mt-1 w-full"><option value="export">Data export</option><option value="correction">Correction</option><option value="consent_withdrawal">Consent withdrawal</option><option value="deletion_anonymization">Deletion / anonymisation review</option></select></label>
                <label v-if="privacy.type === 'consent_withdrawal'" class="text-sm font-medium">Consent to withdraw<select v-model="privacy.details.consent_type" class="cd-input mt-1 w-full" required><option value="marketing">Marketing</option><option value="photography">Photography</option><option value="treatment">Treatment</option></select></label>
