...
<template>
    <AppLayout title="Business setup" :business-label="business.name">
        <PageHeader eyebrow="Business settings" title="Get ready to take bookings" description="Complete one focused section at a time. ClipperDesk saves each section separately, so you can leave and return safely.">
            <template #actions>
        ...
        <SurfaceCard v-show="['hours','services','staff','staff_availability'].includes(activeSection)" id="hours" class="mt-6" title="Create your first bookable service" description="Connect one location, staff member, service, and weekly schedule. You can add exceptions and advanced capacity rules later.">
            <form class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" @submit.prevent="saveFirstPath">
                ...
                <label class="text-sm font-semibold">Time zone<select v-model="availabilityForm.time_zone" required class="cd-input mt-2"><option v-for="zone in referenceData.time_zones" :key="zone" :value="zone">{{ zone.replaceAll('_', ' ') }}</option></select></label>
                ...
                <label class="text-sm font-semibold">Price ({{ business.currency_code }})<input v-model="priceAmount" type="number" min="0" step="0.01" inputmode="decimal" required class="cd-input mt-2" /></label>
        ...
        <div v-show="['hours','services','staff','staff_availability'].includes(activeSection)" class="mt-6 grid gap-6 md:grid-cols-3">
            <SurfaceCard title="Locations & capacity"><p class="text-3xl font-bold text-[var(--brand-primary)]">{{ locations.length }}</p><p class="mt-2 text-sm text-[var(--text-muted)]">Locations with normal/special hours, holidays, closures, and reusable resources.</p></SurfaceCard>
            <SurfaceCard id="services" title="Services & add-ons"><p class="text-3xl font-bold text-[var(--brand-primary)]">{{ services.length }}</p><p class="mt-2 text-sm text-[var(--text-muted)]">Segmented duration, price, tax, deposit, eligibility, staff variants, and resource rules.</p></SurfaceCard>
            <SurfaceCard id="staff" title="Staff & schedules"><p class="text-3xl font-bold text-[var(--brand-primary)]">{{ staff.length }}</p><p class="mt-2 text-sm text-[var(--text-muted)]">Invitations, locations, split shifts, breaks, leave, and temporary changes.</p></SurfaceCard>
        </div>
