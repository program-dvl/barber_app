...
<template>
    <AppLayout title="Inventory" :business-label="page.props.tenant.name">
        ...
        <header class="flex flex-wrap items-end justify-between gap-4 border-b border-[var(--border-subtle)] pb-5">
            <div><p class="text-sm font-semibold text-[var(--brand-primary)]">FR-16 · Append-only stock</p><h1 class="cd-display mt-1 text-4xl text-[var(--text-strong)]">Inventory</h1><p class="mt-2 text-sm text-[var(--text-muted)]">Fresh {{ new Date(freshAt).toLocaleString() }} · {{ timeZone }}</p></div>
