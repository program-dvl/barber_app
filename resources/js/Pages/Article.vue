...
<template>
    <HomeLayout>
        ...
        <section v-if="related.length" class="cd-public-section bg-[var(--surface-subtle)]"><PublicContainer><h2 class="font-display text-3xl font-semibold text-[var(--text-strong)]">Related reviewed reading</h2><div class="mt-8 grid gap-5 md:grid-cols-2"><MarketingCard v-for="item in related" :key="item.id"><h3 class="text-xl font-extrabold text-[var(--text-strong)]">{{ item.title }}</h3><p class="mt-3 text-[var(--text-muted)]">{{ item.excerpt }}</p><Link :href="route('blog.article', item.slug)" class="mt-5 inline-flex min-h-11 items-center font-extrabold text-[var(--brand-primary)] underline-offset-4 hover:underline">Read this article</Link></MarketingCard></div></PublicContainer></section>
    </HomeLayout>
</template>
