...
<template>
    <PublicBookingLayout title="Book an appointment" :current-step="step">
        <template v-if="!business">
            <div class="text-center"><p class="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--brand-primary)]">Book with confidence</p><h1 class="cd-display mt-2 text-4xl text-[var(--text-strong)] sm:text-5xl">Use your business’s booking link</h1><p class="mx-auto mt-3 max-w-xl leading-7 text-[var(--text-muted)]">Open the private booking address shared by your barber, salon, spa, or independent professional.</p></div>
        ...
        <template v-else>
            <header><p class="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--brand-primary)]">{{ business.name }}</p><h1 class="cd-display mt-2 text-4xl leading-tight text-[var(--text-strong)]">Book an appointment</h1><p class="mt-2 text-sm text-[var(--text-muted)]">Times are shown in the selected location’s local time. No account or app needed.</p></header>
