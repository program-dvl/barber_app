...
const clientSearch = ref(''); const clientResults = ref([]); const selectedClient = ref(null); const searchingClients = ref(false);
...
const addForm = useForm({ location: props.location.public_id, service: props.services[0]?.public_id || '', preferred_staff: '', client_mode: 'existing', client: '', client_name: '', client_mobile: '', client_email: '', arrived_at: localInputNow(), notes: '' });
watch(clientSearch, value => {
    clearTimeout(searchTimer); clientResults.value = [];
    if (value.trim().length < 2 || selectedClient.value) return;
    searchTimer = setTimeout(async () => {
        searchingClients.value = true;
        try {
            const url = route('business.walk-ins.clients.search', route().params.business) + `?q=${encodeURIComponent(value.trim())}`;
            const response = await fetch(url, { headers: { Accept: 'application/json' } });
            clientResults.value = response.ok ? (await response.json()).clients : [];
        } finally { searchingClients.value = false; }
    }, 250);
});
const chooseClient = client => { selectedClient.value = client; addForm.client = client.public_id; clientSearch.value = client.name; clientResults.value = []; };
const setClientMode = mode => { addForm.client_mode = mode; addForm.client = ''; selectedClient.value = null; clientSearch.value = ''; clientResults.value = []; addForm.clearErrors(); };
...
<template>
    <AppLayout title="Walk-in queue" :business-label="businessLabel">
            ...
            <SurfaceCard :padding="false" title="Waiting now" :description="`${waiting.length} ${waiting.length === 1 ? 'person' : 'people'} waiting`">
                ...
                <ol v-else class="divide-y divide-[var(--border-subtle)]">
                    <li v-for="entry in waiting" :key="entry.public_id" class="p-4 sm:p-5">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div class="flex gap-3"><span class="grid size-11 shrink-0 place-items-center rounded-full bg-[var(--brand-primary)] text-lg font-bold text-white" :aria-label="`Queue position ${entry.queue_position}`">{{ entry.queue_position }}</span><div><div class="flex flex-wrap items-center gap-2"><h2 class="font-semibold text-[var(--text-strong)]">{{ entry.client_name }}</h2><span class="rounded-full bg-[var(--status-warning-soft)] px-2 py-1 text-xs font-bold uppercase text-[var(--status-warning)]">{{ entry.status }}</span><span v-if="entry.client_public_id" class="rounded-full bg-[var(--status-success-soft)] px-2 py-1 text-xs font-semibold text-[var(--status-success)]">Client linked</span></div><p class="mt-1 flex items-center gap-1 text-sm text-[var(--text-muted)]"><ClockIcon class="size-4" aria-hidden="true" />{{ entry.estimated_wait_minutes === null ? 'No safe slot found today' : `About ${entry.estimated_wait_minutes} min · expected ${localTime(entry.estimated_service_at)}` }}</p><details class="mt-2 text-xs text-[var(--text-muted)]"><summary class="min-h-11 cursor-pointer py-3 font-semibold">Why this estimate?</summary><p>{{ entry.estimate_evidence.queue_entries_ahead }} ahead · {{ entry.estimate_evidence.service_duration_minutes }}-minute service · future appointments and staff capacity checked.</p></details></div></div>
                            <div class="flex w-full flex-col gap-2 sm:w-auto sm:min-w-56"><AppSelect :value="entry.assigned_staff_id || entry.preferred_staff_id || ''" :class="inputClass" aria-label="Assign staff" @change="assign(entry, $event.target.value)"><option value="" disabled>Assign staff</option><option v-for="member in staff" :key="member.public_id" :value="member.public_id">{{ member.display_name }}</option></AppSelect><AppButton class="w-full" :disabled="!entry.assigned_staff_id && !entry.preferred_staff_id" @click="start(entry)">Start service</AppButton><div class="flex gap-2"><AppButton class="flex-1" variant="secondary" @click="notify(entry)"><BellAlertIcon class="size-5" aria-hidden="true" />Notify</AppButton><AppButton class="flex-1" variant="quiet" @click="openLeave(entry)">Remove</AppButton></div></div>
                        </div>
                    </li>
                </ol>
            </SurfaceCard>
        ...
        <AppDialog id="add-walk-in" ref="addDialog" title="Add walk-in" description="Link the client now, then we will calculate a safe wait estimate." confirm-label="Add to queue" :close-on-confirm="false" :confirm-disabled="addForm.processing || (addForm.client_mode === 'existing' && !addForm.client)" @confirm="add"><div class="space-y-4"><p v-if="Object.keys(addForm.errors).length" class="rounded-lg bg-[var(--status-danger-soft)] p-3 text-sm text-[var(--status-danger)]" role="alert">{{ Object.values(addForm.errors)[0] }}</p><div class="grid grid-cols-2 gap-2 rounded-xl bg-[var(--surface-subtle)] p-1" aria-label="Client choice"><button type="button" :aria-pressed="addForm.client_mode === 'existing'" :class="['min-h-11 rounded-lg text-sm font-semibold', addForm.client_mode === 'existing' ? 'bg-white text-[var(--action-primary)] shadow-sm' : 'text-[var(--text-muted)]']" @click="setClientMode('existing')">Existing client</button><button type="button" :aria-pressed="addForm.client_mode === 'new'" :class="['min-h-11 rounded-lg text-sm font-semibold', addForm.client_mode === 'new' ? 'bg-white text-[var(--action-primary)] shadow-sm' : 'text-[var(--text-muted)]']" @click="setClientMode('new')">New client</button></div><div v-if="addForm.client_mode === 'existing'" class="relative"><FormField id="walkin-client-search" label="Find a client" required><input id="walkin-client-search" v-model="clientSearch" :class="inputClass" autocomplete="off" placeholder="Search name, mobile or email" @input="selectedClient && setClientMode('existing')" /></FormField><p v-if="searchingClients" class="mt-2 text-xs text-[var(--text-muted)]">Searching…</p><div v-else-if="clientResults.length" class="mt-2 overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-white shadow-lg"><button v-for="client in clientResults" :key="client.public_id" type="button" class="flex min-h-14 w-full items-center justify-between gap-3 border-b border-[var(--border-subtle)] px-3 text-left last:border-0 hover:bg-[var(--surface-subtle)]" @click="chooseClient(client)"><span><strong class="block text-sm">{{ client.name }}</strong><span class="block text-xs text-[var(--text-muted)]">{{ client.mobile }}</span></span><span class="text-xs text-[var(--text-muted)]">{{ client.email || '' }}</span></button></div><div v-if="selectedClient" class="mt-2 rounded-xl bg-[var(--status-success-soft)] p-3 text-sm text-[var(--status-success)]"><strong>{{ selectedClient.name }}</strong> will be linked to this queue visit.</div><p v-else-if="clientSearch.length >= 2 && !searchingClients && !clientResults.length" class="mt-2 text-xs text-[var(--text-muted)]">No matching client with a mobile number. Choose “New client” to create one now.</p></div><template v-else><FormField id="walkin-name" label="Name" required><input id="walkin-name" v-model="addForm.client_name" :class="inputClass" autocomplete="name" /></FormField><FormField id="walkin-mobile" label="Mobile number" required><PhoneInput id="walkin-mobile" v-model="addForm.client_mobile" required :country="$page.props.tenant?.regional?.country_code || 'IN'" /></FormField><FormField id="walkin-email" label="Email for queue updates"><input id="walkin-email" v-model="addForm.client_email" type="email" autocomplete="email" :class="inputClass" /><span class="mt-1 block text-xs text-[var(--text-muted)]">Appointment and queue emails are on by default unless the client opts out.</span></FormField></template><FormField id="walkin-service" label="Requested service" required><AppSelect id="walkin-service" v-model="addForm.service" :class="inputClass"><option v-for="service in services" :key="service.public_id" :value="service.public_id">{{ service.name }} · {{ service.duration_minutes }} min</option></AppSelect></FormField><FormField id="walkin-staff" label="Preferred staff"><AppSelect id="walkin-staff" v-model="addForm.preferred_staff" :class="inputClass"><option value="">First available</option><option v-for="member in staff" :key="member.public_id" :value="member.public_id">{{ member.display_name }}</option></AppSelect></FormField><FormField id="walkin-arrival" label="Arrival time"><input id="walkin-arrival" v-model="addForm.arrived_at" type="datetime-local" :class="inputClass" /></FormField><FormField id="walkin-notes" label="Internal note"><textarea id="walkin-notes" v-model="addForm.notes" rows="3" :class="inputClass" /></FormField></div></AppDialog>
