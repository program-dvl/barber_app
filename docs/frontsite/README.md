...
| `/terms-of-service` | Counsel-owned terms | Account/legal evaluator | Substantive `0.2-review` draft; operator/legal clauses pending OPEN-13 | Register only after approval | Self; **noindex until counsel approval** | WebPage only after approval | 21 |
| `/privacy-policy` | Counsel-owned privacy notice | Visitor/data subject | Substantive `0.2-review` draft; OPEN-10/13 | Register only after approval | Self; **noindex until counsel approval** | WebPage only after approval | 21 |
| `/refund-policy` | Explain subscription and salon-payment cancellation/refund boundaries | Business buyer, Salon Client, provider reviewer | ADR-020/021, Stripe/Paddle primary guidance; OPEN-13 | Complete a payment only after final approval | Self; **noindex until counsel/Finance/Operations approval** | WebPage only after approval | 21 |
